...
def _generate_summary_llm(book_name: str, chapter: int, chapter_text: str, version_name: str = "Bible") -> str:
    ...
    prompt = (
        ...
        f"Length: about 100 words. Never more than 200. Most chapters need three or four "
        f"sentences — if you are writing a third paragraph you are summarising the wrong "
        f"thing.\n\n"
        f"Stay HIGH LEVEL. Name only the people and events the point depends on. No "
        f"inventories, no counts of livestock or children, no listing each messenger or "
        f"speech in turn, no quoting. If a detail could be removed without changing what "
        f"the chapter is about, remove it — that is most details. A reader should finish "
        f"knowing what the chapter is FOR, not everything that is in it.\n\n"
        f"Do NOT go through the chapter verse by verse, and do NOT restate each verse in "
        f"turn. Retelling the chapter more briefly is not summarising it. Step back far "
        f"enough to say what it is doing and why it is here.\n\n"
