# Scriptures — a Skipperbot app package

This repo is a **Skipperbot app** (app id `scriptures`, entity prefixes
`bv-`/`bb-`/`sbm-`/`shl-`/`sn-`/`rpl-`/`rpp-`, Postgres schema `app_scriptures`). It installs by being dropped
into a Skipperbot platform's `apps/scriptures/` folder.
...
- **Settings**: your own settings auto-scope to `app:scriptures`;
...
- To try the app on a platform: copy/clone this folder to
  `<platform>/apps/scriptures/` and restart the agent.
